...
modelParam::modelParam(arma::mat x_train_,
        ...
        beta = beta_;
        tau_mu = tau_mu_;
        tau_b = arma::vec(n_tree_,arma::fill::ones)*tau_b_;
        tau_b_intercept = tau_b_intercept_;
        tau = tau_;
        a_tau = a_tau_;
        d_tau = d_tau_;
        nu = nu_;
        delta = arma::vec(n_tree_,arma::fill::ones)*delta_;
        diag = arma::eye(B_train_.n_cols,B_train.n_cols);
        a_delta = a_delta_;
        d_delta = d_delta_;
        n_mcmc = n_mcmc_;
        n_burn = n_burn_;
...
void grow(Node* tree, modelParam &data, arma::vec &curr_res, int &t){
        ...
        g_node->left->splineNodeLogLike(data, curr_res, t);
        g_node->right->splineNodeLogLike(data, curr_res, t);
...
void prune(Node* tree, modelParam&data, arma::vec &curr_res, int &t){
        ...
        p_node->splineNodeLogLike(data, curr_res, t);
        // cout << "Error C2" << endl;
...
void change(Node* tree, modelParam &data, arma::vec &curr_res, int &t){
        ...
        c_node->left->splineNodeLogLike(data,curr_res,t);
        c_node->right->splineNodeLogLike(data,curr_res,t);
...
void Node::splineNodeLogLike(modelParam& data, arma::vec &curr_res, int& t){
        ...
        for(int k = 0; k < data.d_var;k++){
                ...
                if(ancestors(k)>0){
                        res_cov = res_cov + (1/data.tau_b(t))*B_.slice(k)*data.diag*B_t_.slice(k); // Do not penalise by tree number of trees factor
                }
                // Test elements
        ...
        if(data.intercept_model){
                res_cov  = ((1/data.tau)*diag_aux + (1/data.tau_b_intercept) + res_cov); // Not penalizing the number of tree in the intercept
        } else {
...
// Update betas
void updateBeta(Node* tree, modelParam &data, int &t){
        ...
        for(int i = 0; i < t_nodes.size();i++){
                ...
                for(int j=0;j<data.d_var;j++){
                        ...
                        if(t_nodes[i]->ancestors(j)>0){
                                ...
                                arma::mat aux_precision_inv = arma::inv(t_nodes[i]->B_t.slice(j)*t_nodes[i]->B.slice(j)+((data.tau_b(t))/data.tau)*data.diag);
                                ...
                                t_nodes[i]->betas.col(j) = arma::vec(t_nodes[i]->betas.n_rows,arma::fill::zeros);
                        }
                }
        }
}
...
void updateTauB(Forest all_trees,
        ...
        // Resetting the beta count prior
        arma::vec beta_count_total = arma::vec(data.n_tree,arma::fill::zeros);
        arma::vec beta_sq_sum_total = arma::vec(data.n_tree,arma::fill::zeros);
        ...
        for(int j = 0; j < data.d_var;j++){
                ...
                for(int t = 0; t< all_trees.trees.size();t++){
                        ...
                        for(int i = 0; i< t_nodes.size(); i++ ){
                                ...
                                if(t_nodes[i]->ancestors(j)>0){
                                        beta_sq_sum_total(t) = beta_sq_sum_total(t) + arma::as_scalar(t_nodes[i]->betas.col(j).t()*data.diag*t_nodes[i]->betas.col(j));
                                        beta_count_total(t) = beta_count_total(t) + t_nodes[i]->betas.n_rows;
                                }
                        }
                }
        }
        ...
        for(int t = 0; t< data.n_tree; t++){
                data.tau_b(t) = R::rgamma((0.5*beta_count_total(t) + 0.5*data.nu),1/(0.5*beta_sq_sum_total(t)+0.5*data.delta(t)*data.nu));
        }
...
void updateDelta(modelParam &data){
        ...
        for(int t = 0; t< data.n_tree; t++){
                data.delta(t) = R::rgamma((0.5*data.nu + data.a_delta),1/(0.5*data.nu*data.tau_b(t)+data.d_delta));
        }
...
Rcpp::List sbart(arma::mat x_train,
        ...
        arma::mat tau_b_post = arma::mat(data.n_tree,n_post,arma::fill::zeros);
        ...
        for(int mcmc_iter = 0;mcmc_iter<data.n_mcmc;mcmc_iter++){
                ...
                for(int t = 0; t<data.n_tree;t++){
                        ...
                        updateBeta(all_forest.trees[t], data,t);
